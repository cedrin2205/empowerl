...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM ict12hands2 WHERE image_url = '$imageToDelete'";
    if (mysqli_query($conn, $deleteQuery)) {
        if (unlink("uploads/$imageToDelete")) {
            header("Location: ICT12hands1.php");
            exit();
        } else {
...
.contain form input[type="file"] {
  ...
  <body>
        ...
        <header>
            ...
            <a href="ICTTEACHER12home.php"><button class="btn">Back</button></a>
        </header>
        ...
        <section>
            ...
            <div class="contain">
                <form action="ICT12uploadhands2.php" method="post" enctype="multipart/form-data">
            ...
            <div class="gallery">
                ...
                include "ictdb12.php";
                $limit = 5; 
                $sql = "SELECT * FROM ict12hands2 ORDER BY id DESC LIMIT $limit";
                $res = mysqli_query($conn, $sql);

                if (mysqli_num_rows($res) > 0) {
                    while ($ict12hands2 = mysqli_fetch_assoc($res)) { ?>
                        <div class="alb">
                            <?php 
                            $fileExtension = pathinfo($ict12hands2['image_url'], PATHINFO_EXTENSION);
                            if ($fileExtension === 'pdf') {
                                echo '<img src="img/pdficon.png" alt="PDF File">';
                            } elseif ($fileExtension === 'doc' || $fileExtension === 'docx') {
                                echo '<img src="img/wordicon.png" alt="Word File">';
                            } else {
                                echo '<img src="uploads/'.$ict12hands2['image_url'].'" alt="Uploaded Image">';
                            }
                            ?>
                            <p><?php echo $ict12hands2['filename']; ?></p>
                            <a class="delete-btn" href="ict12hands1.php?delete_image=<?=$ict12hands2['image_url']?>">
                                <img src="img/remove.png" alt="Delete" width="1" height="1">
                            </a>
                            <a class="download-btn" href="uploads/<?=$ict12hands2['image_url']?>" download="<?=$ict12hands2['image_url']?>">
...
<script>
                ...
                xhttp.open("GET", "ict12hands2.php?mark_as_seen=" + encodeURIComponent(imageUrl), true);
                xhttp.send();
